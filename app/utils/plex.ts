...
class PlexOAuth {
  ...
  public initializeHeaders(): void {
    ...
    this.plexHeaders = {
      ...
      "X-Plex-Device-Name": `${browser.getBrowserName()} (Removarr)`,
  ...
  public async getPin(): Promise<PlexPin> {
    ...
    this.pin = { id: response.id, code: response.code };
    ...
    const executePoll = async (
      ...
      try {
        ...
        if (response?.authToken) {
          this.authToken = response.authToken as string;
          this.closePopup();
          resolve(this.authToken);
        } else if (!response?.authToken && !this.popup?.closed) {
